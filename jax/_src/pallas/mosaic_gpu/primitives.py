...
def _copy_gmem_to_smem_lowering(
  ...
  if is_cp_async:
    ...
    if oob_mode is not None:
      raise ValueError(
          "Only the TMA implementation supports out-of-bounds fill modes"
      )
  ...
  if ctx.module_ctx.lowering_semantics == mgpu.LoweringSemantics.Lane:
    ...
    ctx.launch_ctx.async_copy(
        ...
        oob_mode=OOBFillMode.ZEROS if oob_mode is None else oob_mode,
        implementation=(
  ...
  * ``CopyPartition.REPLICATED``: all CUDA blocks in the cluster load the same
  ...
  Args:
    ...
    oob_mode: The optional out-of-bounds fill mode. Can be
      ...
      ``OOBFillMode.ZEROS``. Only supported with the TMA implementation (Hopper
      and newer).
