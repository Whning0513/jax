...
def _render_markdown(reports, threshold_mb=500.0):
  ...
  lines = [
      f"## :bar_chart: JAX Test Memory Report (>= {threshold_mb:.0f} MB)",
      "",
  ...
  if shown_count == 0:
    lines.append(f"| *(no tests exceeded {threshold_mb:.0f} MB)* | - | - |")
  return "\n".join(lines) + "\n"
...
def main():
  ...
  parser.add_argument(
      ...
      type=float,
      default=750.0,
      help="Only include tests with peak memory >= threshold in MB (default: 750).",
  )
  ...
  markdown = _render_markdown(reports, threshold_mb=args.threshold_mb)
  print(markdown, end="")
