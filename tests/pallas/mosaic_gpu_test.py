...
class PallasTest(jtu.JaxTestCase, metaclass=PallasTestMetaclass):
  ...
  def setUp(
      self,
      *,
      artificial_shared_memory_limit=jtu._SMEM_SIZE_BOUND_FOR_TESTS,
      min_capability="9.0",
  ):
...
class PallasSm80Test(PallasTest):

  def setUp(self):
    super().setUp(min_capability="8.0")
  ...
  def test_copy_gmem_to_smem_dynamic_slice(self):
    ...
    def kernel(x_ref_gmem, o_ref, scratch_ref, barrier_ref):
      ...
      def _(i):
        s = pl.ds(i * 128, 128)
        plgpu.copy_gmem_to_smem(
            x_ref_gmem.at[s], scratch_ref, barrier_ref
        )
        plgpu.barrier_wait(barrier_ref)
  ...
  def test_tiled_copy_gmem_to_smem_squeeze(self):
    self.skip_if_wg_semantics()
    ...
    def kernel(x_ref_gmem, o_ref, scratch_ref, barrier_ref):
      ...
      def _(i):
        plgpu.copy_gmem_to_smem(
            x_ref_gmem.at[i], scratch_ref, barrier_ref
        )
        plgpu.barrier_wait(barrier_ref)
